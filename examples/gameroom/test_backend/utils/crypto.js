 ...
 */
export function hashSHA256(salt, data) {
 ...
 */
export function encrypt(password, privateKey) {
 ...
 */
export function decrypt(password, encryptedPrivateKey) {
...
export function signPayload(payload, privateKey) {
    ...
    message.signature = hexToBytes(signer.sign(JSON.stringify(header)));
 ...
 * @param signer - Wrapper containing the user's keys.
 */
export function signXOPayload(payload, privateKey) {
    const privKey = Secp256k1PrivateKey.fromHex(privateKey);
    const signer = CRYPTO_FACTORY.newSigner(privKey);
    const pubKey = signer.getPublicKey().asBytes();
